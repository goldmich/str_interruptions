...
gnomad_vcf = snakemake.input[1]

curr_chr = 'chr' + os.path.basename(gnomad_vcf).split('.')[1]

interruption_dict = {}
# var_qual_list = []
with pysam.VariantFile(vcf_filename) as vcf_in:
	with pysam.VariantFile(gnomad_vcf) as gnomad_vcf_in:
		for var in vcf_in.fetch():
			...
			for gnomad_var in gnomad_vcf_in.fetch(curr_chr, curr_region[0], curr_region[1]):
				if 'PASS' in gnomad_var.filter and gnomad_var.info['variant_type'] == 'snv':
					max_af = max(gnomad_var.info['AF'])
					pos_within_repeat = (gnomad_var.pos - curr_region[0]) / (curr_region[1] - curr_region[0])
					...
					elif max(gnomad_var.info['AC']) > 1: # rare but not singletons
						n_var[pos_quantile, 1] += 1
					else: # singletons
						n_var[pos_quantile, 0] += 1
						
# 					var_qual_list.append(gnomad_var.qual)
